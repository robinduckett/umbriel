...
When a layer rule sets `blur_shared`, `BlurBackdrop::update`
([`blur_backdrop.cpp`](../../src/scene/blur_backdrop.cpp)) also runs there on
frames with damage. It walks the top layer tree for visible blur nodes that
sample the shared backdrop, then asks for a partial capture of the part whose
blur can have changed. The capture adds damage for that region only, so it must
run before the damage test.
