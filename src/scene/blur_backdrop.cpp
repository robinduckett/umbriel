...
namespace umbriel {
  ...
  namespace {
    ...
    // Areas (layout coordinates) of visible blur nodes that sample the shared capture. A node that is covered, for
    // example by a fullscreen window, needs no capture.
    void collectSharedBlurBoxes(wlr_scene_node* node, std::vector<wlr_box>& boxes) {
      ...
      if (!blur->use_shared_blur || blur->width <= 0 || blur->height <= 0 || !wlr_scene_blur_is_visible(blur)) {
        return;
      }
      int lx = 0;
      int ly = 0;
  ...
  void BlurBackdrop::update(wlr_scene_optimized_blur* node, wlr_scene_output* sceneOutput, wlr_scene_tree* tree) {
