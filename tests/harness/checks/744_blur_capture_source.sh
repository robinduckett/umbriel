#!/usr/bin/env bash
# With appearance.blur.capture_source = "windows", shell surfaces using optimized blur show what is beneath them,
# including windows, and a bar and a panel attached below it blur one continuous backdrop, so their join has no step.
# A bottom-layer square stands in for a window: it is below the top layer like the window stack.
set -euo pipefail

readonly LAYER="${UMBRIEL_LAYER_CLIENT:-./build-debug/tests/layer-client}"
readonly SCREENSHOT="$UMBRIEL_RUNTIME_DIR/blur-capture-source.png"
readonly BASE_CONFIG="$UMBRIEL_RUNTIME_DIR/blur-capture-source-base.toml"
cp "$UMBRIEL_CONFIG" "$BASE_CONFIG"

# Each phase rewrites the whole config: a TOML table may appear only once.
write_config() {
  local capture_source=$1 optimized=$2
...
start_layer() {
  local name=$1
  shift
  local log="$UMBRIEL_RUNTIME_DIR/blur-capture-source-$name.log"
...
# Switched back at runtime, optimized blur captures only the background again.
write_config background true
capture
background_over_window=$(green 90 120 20 20)
if (($(abs $((background_over_window - over_background))) > 3)); then
  echo "capture_source = background still shows the window: g=$background_over_window, background g=$over_background"
  exit 1
fi

echo "capture_source = windows blurs windows behind shell surfaces and joins a bar and panel seamlessly"
